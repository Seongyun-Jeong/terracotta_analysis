var module3 = require('./3'),
  ...
  c = {
    ...
    checkMergeArrayArgs: function (n, c) {
      module3(Array.isArray(n) && Array.isArray(c), 'Tried to merge arrays, instead got %s and %s.', n, c);
    },
    ...
    checkMergeObjectArg: function (c) {
      module3(!n(c) && !Array.isArray(c), 'Tried to merge an object, instead got %s.', c);
    },
    checkMergeIntoObjectArg: function (c) {
      module3(!((n(c) && 'function' != typeof c) || Array.isArray(c)), 'Tried to merge into an object, instead got %s.', c);
    },
    checkMergeLevel: function (n) {
      module3(n < 36, 'Maximum deep merge depth exceeded. You may be attempting to merge circular structures in an unsupported way.');
    },
