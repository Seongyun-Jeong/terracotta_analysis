...
module.exports = module172({
  ...
  arcTo: function (t, n, s, h, o, u, p) {
    if (((h = Math.abs(+h || +s || +n - this.penY)), !(s = Math.abs(+s || +module172 - this.penX)) || !h || (module172 == this.penX && n == this.penY)))
      return this.lineTo(module172, n);
    ...
    var c = this.penX,
      ...
      f = Math.cos(Y),
      M = Math.sin(Y),
      b = (f * (module172 -= c)) / 2 + (M * (n -= l)) / 2,
      T = (-M * module172) / 2 + (f * n) / 2,
      k = s * s * h * h,
      w = h * h * b * b,
      _ = s * s * T * T,
      D = k - _ - w;
    ...
    var B = f / s,
      ...
      I = f / h,
      P = (L * -b + I * -T) ** (B * -b + A * -T),
      N = (L * (module172 - b) + I * (n - T)) ** (B * (module172 - b) + A * (n - T));
    b += c;
    T += l;
    module172 += c;
    n += l;
    ...
    this.onArc(c, l, (this._pivotX = this.penX = module172), (this._pivotY = this.penY = n), b, T, s, h, P, N, !v, p);
    return this;
  },
