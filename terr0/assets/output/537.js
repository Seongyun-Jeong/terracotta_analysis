exports.default = function (t, o) {
  ...
  if (!(N === c.EXTEND))
    N === c.CLAMP
      ? (D = module525.createAnimatedCond(module538.lessThan(t, s[0]), T[0], D))
      : N === c.IDENTITY && (D = module525.createAnimatedCond(module538.lessThan(t, s[0]), t, D));
  if (!(b === c.EXTEND))
    b === c.CLAMP
      ? (D = module525.createAnimatedCond(module538.greaterThan(t, s[s.length - 1]), T[T.length - 1], D))
      : b === c.IDENTITY && (D = module525.createAnimatedCond(module538.greaterThan(t, s[s.length - 1]), t, D));
  return D;
};
...
function h(t, n) {
  for (var u = 1; u < n.length; ++u)
    n[u] instanceof module527.default || n[u - 1] instanceof module527.default || module191.default(n[u] >= n[u - 1], '%s must be monotonically non-decreasing. (%s)', t, n);
}
